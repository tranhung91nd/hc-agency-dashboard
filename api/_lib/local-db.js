...
function jwtSecret() {
  return process.env.LOCAL_DB_JWT_SECRET || process.env.DB_JWT_SECRET || process.env.JWT_SECRET || '';
}
...
function sessionJson(row) {
  const secret = jwtSecret();
  if (!secret) throw httpError(500, 'Missing LOCAL_DB_JWT_SECRET');
...
function safeStoragePath(bucket, objectName) {
  const base = path.resolve(process.env.LOCAL_DB_STORAGE_DIR || path.join(process.cwd(), 'storage'));
...
router.get('/health', (_req, res) => res.json({ ok: true, service: 'local-db-bridge' }));
...
router.use((err, _req, res, _next) => {
  ...
  const body = {
    code: err.code || (status >= 500 ? 'LOCAL_DB_ERROR' : 'LOCAL_DB_REQUEST_ERROR'),
    details: null,
    hint: null,
    message: err.message || String(err),
  };
  if (status >= 500) console.error('[local-db]', err);
