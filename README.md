...
## ChatGPT Plus OAuth (`/api/chatgpt-*`)

Cho phép HC AI panel dùng subscription ChatGPT Plus thay vì trả tiền API key. PKCE flow của Codex CLI — token lưu Supabase, tự refresh.

### Setup 1 lần

**1. Migration:** chạy `migrations/2026-05-10_oauth_tokens.sql` trên Supabase SQL Editor.

**2. Connect:** vào dashboard → Admin → Cài đặt API Key → card "Kết nối ChatGPT Plus" → bấm Kết nối → đăng nhập ChatGPT trên tab mới → copy URL `localhost:1455/auth/callback?code=...` → paste vào textarea → Hoàn tất.

### ⚠ Cloudflare WAF block — phải dùng proxy local

ChatGPT backend (`chatgpt.com`) chặn request từ Vercel datacenter IP qua Cloudflare WAF. Phải chạy proxy ở máy local (IP nhà mạng pass CF), Vercel function forward qua tunnel.

**1. Chạy proxy local:**
```bash
HC_PROXY_SECRET=$(openssl rand -hex 32) node scripts/chatgpt-proxy.js
```
In ra secret — copy lưu lại.

**2. Expose qua tunnel:**
```bash
# Cách 1: ngrok (đăng ký free account, lấy authtoken)
ngrok config add-authtoken <token>
ngrok http 8787

# Cách 2: localtunnel (không cần đăng ký, URL ngẫu nhiên)
npx localtunnel --port 8787

# Cách 3: cloudflared (cần Cloudflare account, URL stable)
cloudflared tunnel --url http://localhost:8787
```
Copy URL (vd `https://abc123.ngrok.io`).

**3. Set env trên Vercel** (Project Settings → Environment Variables):

| Key | Value |
|---|---|
| `CHATGPT_PROXY_URL` | URL tunnel ở bước 2 |
| `HC_PROXY_SECRET` | Secret ở bước 1 |

→ Redeploy.

**4. Khi nào cần dùng AI panel với subscription:** mở terminal chạy proxy + tunnel. Tắt → fallback đổi model trong dropdown sang GPT-5.4 Mini (API key) hoặc Claude.

### Tradeoff

- **Pro:** Dùng subscription Plus đã trả tiền, không tốn token API
- **Con:** Phải bật máy + tunnel khi dùng. URL ngrok free đổi mỗi restart (phải update env). Có thể move proxy lên VPS rẻ ($3-5/tháng) chạy 24/7 với cloudflared tunnel stable URL.
- **Rủi ro:** endpoint `chatgpt.com/backend-api/codex/responses` không chính thức, OpenAI có thể đổi/khoá account hiếm khi.
