-- ═══════════════════════════════════════════════════════════════
-- Migration: RPC public report — khách xem báo cáo Ads daily qua URL
--
-- URL pattern: ?report=<client_id>&token=<share_token>
-- Function trả về: client info + daily_spend + campaign_daily_mess
...
DECLARE
  ...
  v_mess jsonb;
BEGIN
  -- 1. Verify token + lấy client (chỉ trả tên + id, KHÔNG trả các field nhạy cảm như share_token, deposit, …)
  ...
  -- 2. Ad accounts của khách (cứng client_id HOẶC qua assignment)
  ...
  -- 4. Daily spend 90 ngày gần nhất, CHỈ TKQC khách này
  ...
  -- 5. Campaign daily mess 90 ngày gần nhất, CHỈ TKQC khách này
  SELECT coalesce(jsonb_agg(jsonb_build_object('report_date',cm.report_date,'ad_account_id',cm.ad_account_id,'mess_count',cm.mess_count,'lead_count',cm.lead_count)), '[]'::jsonb) INTO v_mess
...
-- ═══ KIỂM TRA ═══
-- SELECT get_public_client_report('<client_id>', '<token>');
