-- ═══════════════════════════════════════════════════════════════
-- Migration: thêm ad_daily_post vào RPC public report
--
-- Mục đích: link share báo cáo gửi khách (vd 4Cats) cũng thấy được
...
BEGIN
  ...
  -- ad_daily_post: bảng Hiệu quả theo bài chạy × tuần (mới)
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'ad_account_id', ap.ad_account_id,
    'ad_id', ap.ad_id,
    'report_date', ap.report_date,
    'ad_name', ap.ad_name,
    'campaign_id', ap.campaign_id,
    'campaign_name', ap.campaign_name,
    'post_id', ap.post_id,
    'post_url', ap.post_url,
    'thumbnail_url', ap.thumbnail_url,
    'spend', ap.spend,
    'mess_count', ap.mess_count,
    'comment_count', ap.comment_count,
    'lead_count', ap.lead_count,
    'checkout_count', ap.checkout_count
  )), '[]'::jsonb) INTO v_post
  FROM ad_daily_post ap
  WHERE ap.report_date >= (current_date - interval '90 days')::date
    AND ap.ad_account_id IN (
      SELECT id FROM ad_account WHERE client_id = p_client_id
      UNION
      SELECT ad_account_id FROM assignment WHERE client_id = p_client_id
    );
